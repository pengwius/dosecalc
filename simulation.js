class SimulationApp {
  constructor() {
    ...
    this.elements = {
      ...
      tripProtocolSelect: document.getElementById("tripProtocolSelect"),
      tripMode: document.getElementById("tripMode"),
      calculateTrip: document.getElementById("calculateTrip"),
      tripPlanResult: document.getElementById("tripPlanResult"),
      tripSuggestionText: document.getElementById("tripSuggestionText"),
      applyTripPlan: document.getElementById("applyTripPlan"),
      revertTripPlan: document.getElementById("revertTripPlan"),
      tripModal: document.getElementById("tripModal"),
      closeTripModal: document.getElementById("closeTripModal"),
      modalCloseBtn: document.getElementById("modalCloseBtn"),
      modalApplyTrip: document.getElementById("modalApplyTrip"),
      tripModalSuggestion: document.getElementById("tripModalSuggestion"),
      showModalE2Target: document.getElementById("showModalE2Target"),
      showModalTTarget: document.getElementById("showModalTTarget"),
      tripDatesList: document.getElementById("tripDatesList"),
  ...
  bindGlobalEvents() {
    ...
    this.elements.calculateTrip.onclick = () => this.calculateTripAdjustment();
    this.elements.applyTripPlan.onclick = () => this.applyTripPlan();
    this.elements.revertTripPlan.onclick = () => this.undoTripPlan();
    this.elements.closeTripModal.onclick = () =>
      (this.elements.tripModal.style.display = "none");
    this.elements.modalCloseBtn.onclick = () =>
      (this.elements.tripModal.style.display = "none");
    this.elements.modalApplyTrip.onclick = () => this.applyTripPlan();
    this.elements.showModalE2Target.onchange = () => this.renderTripChart(this.protocols.find(p => p.id === this.currentTripPlan?.protocolId));
    this.elements.showModalTTarget.onchange = () => this.renderTripChart(this.protocols.find(p => p.id === this.currentTripPlan?.protocolId));
  }
  ...
  updateTripProtocolSelect() {
    ...
    this.protocols.forEach((p, i) => {
      ...
      opt.value = p.id;
      const hormone = this.substances[p.categorySelect.value]?.categoryName || "Unknown";
      const ester = this.substances[p.categorySelect.value]?.esters?.[p.typeSelect.value]?.name || "Unknown";
      opt.textContent = `Protocol ${i + 1}: ${hormone} (${ester})`;
  ...
  createProtocol(data = null) {
    ...
    container.innerHTML = `
      ...
      <div class="regular-schedule-options">
        <div class="input-row">
          <div class="input-group"><label>Schedule Type</label><select data-role="schedule-type"><option value="interval">Every X days/hours</option><option value="weekdays">Days of week</option><option value="monthdays">Days of month</option></select></div>
        </div>
        ...
        <div class="schedule-type-weekdays" style="display:none">
          <div class="input-row">
             <div class="input-group"><label>Days of week</label><div class="weekdays-grid" style="display:flex; gap:8px; flex-wrap:wrap">
    ...
    const p = {
      ...
      scheduleType: container.querySelector('[data-role="schedule-type"]'),
      doseInterval: container.querySelector('[data-role="interval"]'),
      intervalUnit: container.querySelector('[data-role="interval-unit"]'),
      referenceDate: container.querySelector('[data-role="reference-date"]'),
      weekDays: container.querySelectorAll('.weekdays-grid input'),
      biWeekly: container.querySelector('[data-role="bi-weekly"]'),
      monthDays: container.querySelector('[data-role="month-days"]'),
      ...
      sections: {
        interval: container.querySelector('.schedule-type-interval'),
        weekdays: container.querySelector('.schedule-type-weekdays'),
        monthdays: container.querySelector('.schedule-type-monthdays')
      }
    };
    ...
    if (data) {
      ...
      p.scheduleType.value = data.scheduleType || 'interval';
      p.doseInterval.value = data.interval || 7;
      p.intervalUnit.value = data.intervalUnit || 'days';
      p.referenceDate.value = data.referenceDate || "";
      if (data.weekDays) p.weekDays.forEach(cb => cb.checked = data.weekDays.includes(parseInt(cb.value)));
      if (p.biWeekly) p.biWeekly.checked = data.biWeekly || false;
      if (p.monthDays) p.monthDays.value = data.monthDays || "";
      ...
      this.updateScheduleVisibility(p);
    } else {
      this.updateTypeOptions(p);
    ...
    p.scheduleType.onchange = () => {
      this.updateScheduleVisibility(p);
      this.updateTripProtocolSelect();
      this.updateSimulation();
    };
    p.typeSelect.onchange = () => {
      this.updateTripProtocolSelect();
      this.updateSimulation();
    };
    p.doseAmount.oninput = () => {
      this.updateTripProtocolSelect();
      this.updateSimulation();
    };
    p.doseInterval.oninput = () => this.updateSimulation();
    p.intervalUnit.onchange = () => this.updateSimulation();
    p.referenceDate.onchange = () => this.updateSimulation();
    p.monthDays.oninput = () => this.updateSimulation();
    if (p.biWeekly) p.biWeekly.onchange = () => this.updateSimulation();
    p.weekDays.forEach(cb => cb.onchange = () => this.updateSimulation());
  ...
  updateScheduleVisibility(p) {
    Object.keys(p.sections).forEach(k => p.sections[k].style.display = 'none');
    if (p.sections[p.scheduleType.value]) p.sections[p.scheduleType.value].style.display = 'block';
  }
  ...
  calculateProtocolLevels(
    ...
    if (this.elements.steadyState.checked && doseEvents.length > 0) {
      const firstEvent = doseEvents[0];
      const interval = protocolInterval || 7;
      const baseDose = (protocolDose !== undefined && !isNaN(protocolDose)) ? protocolDose : firstEvent.dose;
      for (let i = 1; i <= 50; i++) {
        extendedEvents.push({ time: firstEvent.time - i * interval, dose: baseDose });
      }
    }
  ...
  saveState() {
    const state = {
      ...
      protocols: this.protocols.map((p) => ({
        ...
        scheduleType: p.scheduleType.value,
        interval: p.doseInterval.value,
        intervalUnit: p.intervalUnit.value,
        referenceDate: p.referenceDate.value,
        weekDays: Array.from(p.weekDays).filter(cb => cb.checked).map(cb => parseInt(cb.value)),
        biWeekly: p.biWeekly.checked,
        monthDays: p.monthDays.value,
  ...
  getDayOffset(dateStr) {
    ...
    const startDate = this.parseDateLocal(start);
  ...
  buildDoseEvents(p, totalDays) {
    ...
    const simulationStart = this.parseDateLocal(startDateVal);
    ...
    const scheduleType = p.scheduleType.value;

    if (scheduleType === 'interval') {
      const interval = parseFloat(p.doseInterval.value) / (p.intervalUnit.value === "hours" ? 24 : 1);
      const refDateStr = p.referenceDate.value;
      let firstT = 0;
      if (refDateStr) {
        firstT = (this.parseDateLocal(refDateStr) - simulationStart) / (1000 * 60 * 60 * 24);
      }
      if (!isNaN(dose) && !isNaN(interval) && interval > 0) {
        let t = firstT;
        while (t < 0) t += interval;
        for (; t <= totalDays; t += interval) {
           if (t >= 0) events.push({ time: t, dose });
        }
      }
    } else if (scheduleType === 'weekdays') {
      const selectedDays = Array.from(p.weekDays).filter(cb => cb.checked).map(cb => parseInt(cb.value));
      const biWeekly = p.biWeekly.checked;
      const refDateStr = p.referenceDate.value || startDateVal;
      const refDate = this.parseDateLocal(refDateStr);
      refDate.setHours(0, 0, 0, 0);

      for (let t = 0; t <= totalDays; t++) {
        const current = new Date(simulationStart);
        current.setDate(current.getDate() + t);
        current.setHours(0, 0, 0, 0);
        
        if (selectedDays.includes(current.getDay())) {
           if (biWeekly) {
              const diffDays = Math.round((current - refDate) / (1000 * 60 * 60 * 24));
      ...
      const days = p.monthDays.value.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n));
      for (let t = 0; t <= totalDays; t++) {
        const current = new Date(simulationStart);
        current.setDate(current.getDate() + t);
  ...
  calculateTripAdjustment() {
    ...
    const p = this.protocols.find(proto => proto.id === protocolId);
    if (!p) return;

    const tripStartDate = this.parseDateLocal(startStr);
    const tripEndDate = this.parseDateLocal(endStr);
    if (isNaN(tripStartDate) || isNaN(tripEndDate)) {
      alert("Invalid trip dates.");
      return;
    }

    const result = this.findTripEvents(p, tripStartDate, tripEndDate);
    const missed = result.missed;
    const preTripDose = result.preTripDose;
    ...
    const totalMissedDose = missed.reduce((sum, e) => sum + e.dose, 0);

    this.currentTripPlan = {
      protocolId,
      preTripDoseTime: preTripDose.time,
      returnDateOffset: result.returnDateOffset,
      mode,
    ...
    if (mode === 'single') {
      ...
      this.currentTripPlan.suggestedPreDose = Math.round((preTripDose.dose + extra) * 10) / 10;
    } else {
      const extraEach = totalMissedDose * 0.5;
      this.currentTripPlan.suggestedPreExtra = Math.round(extraEach * 10) / 10;
      this.currentTripPlan.suggestedPostExtra = Math.round(extraEach * 10) / 10;
    }

    this.showTripModal(p, missed, preTripDose, mode, totalMissedDose);
  }

  findTripEvents(p, tripStartDate, tripEndDate) {
    const simStart = this.parseDateLocal(this.elements.startDate.value);
    const toOffset = (d) => (d - simStart) / 86400000;
    const retDateOffset = toOffset(tripEndDate);

    const missed = [];
    let preTripDose = null;
    const doseVal = parseFloat(p.doseAmount.value);
    const catKey = p.categorySelect.value;
    const type = this.substances[catKey]?.esters?.[p.typeSelect.value];

    if (type?.type === "continuous") {
      const perDose = doseVal / 3;
      for (let t = tripStartDate.getTime(); t <= tripEndDate.getTime(); t += 8 * 3600000) {
        missed.push({ time: toOffset(new Date(t)), dose: perDose });
      }
      return { missed, preTripDose: { time: toOffset(new Date(tripStartDate.getTime() - 8 * 3600000)), dose: perDose }, returnDateOffset: retDateOffset };
    }
    ...
    if (p.customToggle.checked) {
      ...
      p.customList.querySelectorAll(".custom-dose-row").forEach(row => {
        const dStr = row.querySelector('[data-role="day"]').value;
        const v = parseFloat(row.querySelector('[data-role="dose"]').value);
        const d = this.parseDateLocal(dStr);
        if (!isNaN(d) && !isNaN(v)) {
          if (d >= tripStartDate && d <= tripEndDate) missed.push({ time: toOffset(d), dose: v });
          if (d < tripStartDate && (!lastBefore || d > lastBefore.date)) lastBefore = { date: d, dose: v };
        }
      });
      if (lastBefore) preTripDose = { time: toOffset(lastBefore.date), dose: lastBefore.dose };
      return { missed, preTripDose, returnDateOffset: retDateOffset };
    }

    const scanStart = new Date(tripStartDate);
    scanStart.setDate(scanStart.getDate() - 365);

    if (isNaN(doseVal)) return { missed, preTripDose: null, returnDateOffset: retDateOffset };

    if (p.scheduleType.value === 'interval') {
      const interval = parseFloat(p.doseInterval.value) / (p.intervalUnit.value === "hours" ? 24 : 1);
      const refDate = this.parseDateLocal(p.referenceDate.value || this.elements.startDate.value);
      if (!isNaN(refDate) && interval > 0) {
        let t = new Date(refDate);
        while (t > scanStart) t.setDate(t.getDate() - interval);
        let lastBefore = null;
        while (t <= tripEndDate) {
          if (t >= tripStartDate) {
            missed.push({ time: toOffset(new Date(t)), dose: doseVal });
          } else {
            lastBefore = { date: new Date(t), dose: doseVal };
          }
          t.setDate(t.getDate() + interval);
        }
        if (lastBefore) preTripDose = { time: toOffset(lastBefore.date), dose: lastBefore.dose };
      }
    } else if (p.scheduleType.value === 'weekdays') {
      const selectedDays = Array.from(p.weekDays).filter(cb => cb.checked).map(cb => parseInt(cb.value));
      const biWeekly = p.biWeekly.checked;
      const refDate = this.parseDateLocal(p.referenceDate.value || this.elements.startDate.value);
      for (let d = new Date(tripStartDate); d <= tripEndDate; d.setDate(d.getDate() + 1)) {
        if (selectedDays.includes(d.getDay())) {
          ...
          if (!biWeekly || Math.floor(diffDays / 7) % 2 === 0) missed.push({ time: toOffset(new Date(d)), dose: doseVal });
        }
      }
      for (let d = new Date(tripStartDate); d > scanStart; d.setDate(d.getDate() - 1)) {
        if (selectedDays.includes(d.getDay())) {
          const diffDays = Math.round((d - refDate) / 86400000);
          if (!biWeekly || Math.floor(diffDays / 7) % 2 === 0) { preTripDose = { time: toOffset(new Date(d)), dose: doseVal }; break; }
        }
      }
    } else if (p.scheduleType.value === 'monthdays') {
      const days = p.monthDays.value.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n));
      for (let d = new Date(tripStartDate); d <= tripEndDate; d.setDate(d.getDate() + 1)) {
        if (days.includes(d.getDate())) missed.push({ time: toOffset(new Date(d)), dose: doseVal });
      }
      for (let d = new Date(tripStartDate); d > scanStart; d.setDate(d.getDate() - 1)) {
        if (days.includes(d.getDate())) { preTripDose = { time: toOffset(new Date(d)), dose: doseVal }; break; }
      }
    }
  ...
  showTripModal(p, missed, preTripDose, mode, totalMissedDose) {
    const modal = this.elements.tripModal;
    modal.style.display = "flex";

    const tripStart = this.getDayOffset(this.elements.tripStart.value);
    const tripEnd = this.getDayOffset(this.elements.tripEnd.value);

    let suggestion = "";
    if (mode === "single") {
      suggestion = `Recommended: Increase dose on ${this.getDateFromOffset(
        this.currentTripPlan.preTripDoseTime,
      )} to ${this.currentTripPlan.suggestedPreDose}mg.`;
    } else {
      suggestion = `Recommended: Extra ${
        this.currentTripPlan.suggestedPreExtra
      }mg on ${this.getDateFromOffset(
        tripStart - 1,
      )} AND ${
        this.currentTripPlan.suggestedPostExtra
      }mg on ${this.getDateFromOffset(tripEnd + 1)}.`;
    }
    this.elements.tripModalSuggestion.textContent = suggestion;

    const datesList = this.elements.tripDatesList;
    datesList.innerHTML = "";

    const items = [];
    if (mode === "single") {
      items.push({
        time: this.currentTripPlan.preTripDoseTime,
        dose: this.currentTripPlan.suggestedPreDose,
        note: "(Adjusted)",
      });
    } else {
      items.push({
        time: tripStart - 1,
        dose: this.currentTripPlan.suggestedPreExtra,
        note: "(New Extra Dose)",
      });
      items.push({
        time: tripEnd + 1,
        dose: this.currentTripPlan.suggestedPostExtra,
        note: "(New Extra Dose)",
      });
    }
    ...
    items.forEach((item) => {
      ...
      div.innerHTML = `<strong>${this.getDateFromOffset(
        item.time,
      )}</strong><br>Dose: ${item.dose}mg <br><small>${item.note}</small>`;
      datesList.appendChild(div);
    });

    this.renderTripChart(p);
  }
  ...
  renderTripChart(p) {
    ...
    for (let t = chartStart; t <= chartEnd; t += outputStep) {
      labels.push(this.getDateFromOffset(t));
    }
    ...
    const getLevels = (events) => {
      ...
      const data = new Array(sampleCount).fill(0);
      const ka = type.ka, ke = type.ke, scale = type.scale;
      const weightFactor = 70 / weightKg;
      let extendedEvents = [...events];
      const interval = parseFloat(p.doseInterval.value) || 7;
      for (let i = 1; i <= 150; i++) {
        extendedEvents.push({ time: events[0].time - i * interval, dose: events[0].dose });
      }
      for (let i = 0; i < sampleCount; i++) {
    ...
    const originalEvents = this.buildDoseEvents(p, chartEnd + 10);
    let originalLevels = getLevels(originalEvents);
    const adjustedEvents = originalEvents.filter((e) => e.time < tripStart || e.time > tripEnd);
    if (this.currentTripPlan.mode === "single") {
      const pre = adjustedEvents.find((e) => e.time === this.currentTripPlan.preTripDoseTime);
      if (pre) pre.dose = this.currentTripPlan.suggestedPreDose;
    } else {
      adjustedEvents.push({ time: tripStart - 1, dose: this.currentTripPlan.suggestedPreExtra });
      adjustedEvents.push({ time: tripEnd + 1, dose: this.currentTripPlan.suggestedPostExtra });
    }
    let adjustedLevels = getLevels(adjustedEvents);

    if (this.elements.useCalibration.checked) {
      const mE2 = parseFloat(this.elements.e2Measured.value);
      const mT = parseFloat(this.elements.tMeasured.value);
      let calibFactor = 1.0;
      if (catKey === "estradiol" && !isNaN(mE2)) calibFactor = mE2 / (originalLevels[0] || 1);
      if (catKey === "testosterone" && !isNaN(mT)) calibFactor = mT / (originalLevels[0] || 1);
      originalLevels = originalLevels.map((v) => v * calibFactor);
      adjustedLevels = adjustedLevels.map((v) => v * calibFactor);
    }

    const annotations = {
      tripBox: {
        type: "box",
        xMin: this.getDateFromOffset(tripStart),
        xMax: this.getDateFromOffset(tripEnd),
    ...
    this.tripChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => `${ctx.dataset.label}: ${Math.round(ctx.raw)} ${p.bloodUnit || ""}`,
            },
          },
        },
        scales: {
          x: { ticks: { color: colors.text, maxTicksLimit: 10 } },
          y: { 
            beginAtZero: false, 
            title: { display: true, text: p.bloodUnit, color: colors.text },
            ticks: { color: colors.text } 
          },
        },
      },
  ...
  applyTripPlan() {
    if (!this.currentTripPlan) return;
    const p = this.protocols.find(proto => proto.id === this.currentTripPlan.protocolId);
    if (!p) return;

    this.previousState = JSON.stringify(this.protocols.map(p => ({
        category: p.categorySelect.value,
        type: p.typeSelect.value,
        dose: p.doseAmount.value,
        scheduleType: p.scheduleType.value,
        ...
        monthDays: p.monthDays.value,
        customToggle: p.customToggle.checked,
        customDoses: Array.from(p.customList.querySelectorAll(".custom-dose-row")).map((row) => ({
          day: row.querySelector('[data-role="day"]').value,
          dose: row.querySelector('[data-role="dose"]').value,
        })),
    })));
    ...
    let tripStart = this.getDayOffset(this.elements.tripStart.value);
    let tripEnd = this.getDayOffset(this.elements.tripEnd.value);

    if (tripStart < preBuffer) {
      const shift = preBuffer - Math.floor(tripStart);
      const d = this.parseDateLocal(this.elements.startDate.value);
      if (!isNaN(d.getTime())) {
        d.setDate(d.getDate() - shift);
        this.elements.startDate.value = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
        tripStart += shift;
        tripEnd += shift;
        this.currentTripPlan.preTripDoseTime += shift;
        this.currentTripPlan.returnDateOffset += shift;
      }
    }
    ...
    const events = this.buildDoseEvents(p, extendedDays);

    p.customToggle.checked = true;
    p.container.classList.add("protocol-card--custom");

    const filtered = events.filter(e => e.time < tripStart || e.time > tripEnd);

    if (this.currentTripPlan.mode === 'single') {
      const preDose = filtered.find(e => e.time === this.currentTripPlan.preTripDoseTime);
      if (preDose) preDose.dose = this.currentTripPlan.suggestedPreDose;
    } else {
      filtered.push({ time: tripStart - 1, dose: this.currentTripPlan.suggestedPreExtra });
      filtered.push({ time: tripEnd + 1, dose: this.currentTripPlan.suggestedPostExtra });
    }

    p.customList.innerHTML = "";
    filtered.sort((a, b) => a.time - b.time).forEach(e => {
      this.addCustomDoseRow(p, { day: this.getDateFromOffset(e.time), dose: e.dose });
    });
  ...
  undoTripPlan() {
    ...
    this.protocols.forEach(p => p.container.remove());
    this.protocols = [];
  ...
  getDateFromOffset(offset) {
    const start = this.elements.startDate.value;
    if (!start) return Math.round(offset);
    const d = this.parseDateLocal(start);
    d.setDate(d.getDate() + offset);
    return `${String(d.getDate()).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
  }

  getTargetAnnotations(hormone, e2VisibleOverride = null, tVisibleOverride = null) {
    const ann = {};
    const e2Show = e2VisibleOverride !== null ? e2VisibleOverride : this.elements.showE2Target.checked;
    const tShow = tVisibleOverride !== null ? tVisibleOverride : this.elements.showTTarget.checked;

    if ((hormone === "Estradiol" || hormone === "both") && e2Show) {
      const min = parseFloat(this.elements.e2TargetMin.value);
      const max = parseFloat(this.elements.e2TargetMax.value);
      if (!isNaN(min) && !isNaN(max)) {
        ann.e2Box = {
    ...
    if ((hormone === "Testosterone" || hormone === "both") && tShow) {
      const min = parseFloat(this.elements.tTargetMin.value);
      const max = parseFloat(this.elements.tTargetMax.value);
      if (!isNaN(min) && !isNaN(max)) {
        ann.tBox = {
